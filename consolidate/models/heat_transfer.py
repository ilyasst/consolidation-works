...
class HeatTransfer:
    ...
    def do_timestep_cond_conv(self, uu,uuold):
        ...
        # import pdb; pdb.set_trace()
        # uu[2:-2, 2:-2] = uuold[2:-2, 2:-2] + (self.dt/(self.rho[2:-2, 2:-2]*self.cp[2:-2, 2:-2]))*(((-1/12*(self.ky[4:, 2:-2] - self.ky[:-4, 2:-2]) + 2/3*(self.ky[3:-1, 2:-2] - self.ky[1:-3, 2:-2]))/(self.dy[0:-3, 2:-2])) * ((-1/12*(uuold[4:, 2:-2] - uuold[:-4, 2:-2]) + 2/3*(uuold[3:-1, 2:-2] - uuold[1:-3, 2:-2]))/(self.dy[0:-3, 2:-2])) + self.ky[2:-2, 2:-2] * (-1/12*(uuold[4:, 2:-2]+ uuold[:-4, 2:-2]) +4/3*(uuold[3:-1, 2:-2]+uuold[1:-3, 2:-2]) - 5/2*uuold[2:-2, 2:-2])/(self.dy2[0:-3, 2:-2]) + self.kx[2:-2, 2:-2] * (-1/12*(uuold[2:-2, 4:] + uuold[2:-2,:-4]) + 4/3*(uuold[ 2:-2, 3:-1] + uuold[2:-2, 1:-3])  -5/2*uuold[2:-2, 2:-2] )/(self.dx2[0:-3, 2:-2]) )
        ...
        # import pdb; pdb.set_trace()
        # uu[3:-3, 3:-3] = uuold[3:-3, 3:-3] + (self.dt/(self.rho[3:-3, 3:-3]*self.cp[3:-3, 3:-3]))*(((1/60*(self.ky[6:, 3:-3] - self.ky[:-6,3:-3]) -3/20*(self.ky[5:-1, 3:-3] - self.ky[1:-5,3:-3]) +3/4*(self.ky[4:-2, 3:-3] - self.ky[2:-4, 3:-3]))/(self.dy[0:-5, 3:-3])) * ((1/60*(uuold[6:, 3:-3] -uuold[:-6, 3:-3]) -3/20*(uuold[5:-1, 3:-3]- uuold[1:-5,3:-3]) +3/4*(uuold[4:-2, 3:-3] - uuold[2:-4, 3:-3]) )/(self.dy[0:-5, 3:-3])) + self.ky[3:-3, 3:-3] * (1/90*(uuold[6:, 3:-3] + uuold[:-6, 3:-3]) -3/20*(uuold[5:-1, 3:-3] + uuold[1:-5, 3:-3]) + 3/2*(uuold[4:-2, 3:-3] +uuold[2:-4, 3:-3]) -49/18*uuold[3:-3, 3:-3])/(self.dy2[0:-5, 3:-3]) + self.kx[3:-3, 3:-3] * (1/90*(uuold[3:-3, 6: ] + uuold[3:-3,:-6]) -3/20*(uuold[3:-3, 5:-1] + uuold[3:-3, 1:-5]) +3/2*(uuold[3:-3, 4:-2] + uuold[3:-3, 2:-4]) -49/18*uuold[3:-3, 3:-3])/(self.dx2[0:-5, 3:-3]) )
